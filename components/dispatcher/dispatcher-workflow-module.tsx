...
function WorkflowActions({ workOrder }: { workOrder: DispatcherWorkOrder }) {
  ...
  const markApproved = () => {
    queryClient.setQueryData<DispatcherWorkOrder[]>(["dispatcher", "work-orders"], (current) =>
...
function DispatcherWorkspace() {
  ...
  const mechanics = useMemo(() => Array.from(new Set(data.map((item) => item.mechanic))), [data]);
