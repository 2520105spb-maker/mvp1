...
function useStockItems() {
  return useQuery<StockItem[]>({ queryKey: ["warehouse", "stock-items"], queryFn: fetchStockItems });
}
...
function WarehouseWorkspace() {
  ...
  const warehouses = useMemo<string[]>(() => Array.from(new Set(data.map((item) => item.warehouse.name))), [data]);
  const categories = useMemo<string[]>(() => Array.from(new Set(data.map((item) => item.category))), [data]);
